...
import { Search, Home, MessageSquare, Users, Settings, LogOut, Menu, X, Send } from 'lucide-react';
import { useSession } from 'next-auth/react';
  ...
  return (
    <div className="flex w-full bg-gray-50 dark:bg-[#0d0d0d] dark:text-white">
      ...
      <div className="w-[350px] bg-white dark:bg-[#0d0d0d] dark:text-white border-r light:border-gray-200 flex flex-col">
        <div className="p-4 border-b light:border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold dark:text-white text-gray-800">Messages</h2>
            <button
        ...
        <div className="overflow-y-auto flex-1">
          {users.map(user => (
            <div key={user.id} className="flex items-center p-4 light:hover:bg-gray-50 cursor-pointer border-b light:border-gray-100">
              <div className="relative">
              ...
              <div className="ml-4 flex-1">
                <div className="flex justify-between">
                  <h3 className="font-semibold dark:text-white text-gray-900">{user.name}</h3>
                  <span className="text-sm dark:text-white text-gray-500">{user.time}</span>
                </div>
                <div className="flex justify-between">
                  <p className="text-sm dark:text-white text-gray-500 truncate">{user.lastMessage}</p>
                  {user.unread > 0 && (
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4 flex-1">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="font-semibold dark:text-white text-gray-900">John Doe</h2>
            ...
            <div key={msg.id} className={`flex ${msg.isSent ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex ${msg.isSent ? 'flex-row-reverse' : 'flex-row'} items-end space-x-2`}>
                ...
                <div className={`max-w-[70%] rounded-lg p-3 ${
                  msg.isSent ? 'bg-blue-500 text-white ml-2' : 'bg-white dark:bg-transparent dark:text-white border text-gray-900 mr-2'
                }`}>
                  <p>{msg.text}</p>
      ...
      {/* <div className="text-center flex flex-1 justify-center items-center flex-col gap-4">
        <h2 className="text-2xl font-bold dark:text-white text-gray-700">Select a conversation</h2>
        <p className="dark:text-white text-gray-500">Choose a chat from the list to start messaging.</p>
      </div> */}
    </div>
  );
};
