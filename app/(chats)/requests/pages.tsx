...
const ChatInterface = () => {
    ...
    const { data: sentFriendRequests, isLoading: sentFriendRequestLoading } = useQuery({
        queryKey: ["sentfriendRequest"],
        queryFn: getSendFriendRequests,
    });


    const { data, isLoading: friendRequestLoading } = useQuery({
    ...
    if (isLoading || friendRequestLoading || sentFriendRequestLoading) {
        return (
            <Loader />
        );
    }
        ...
        <div className="flex w-full h-screen bg-gray-50 dark:bg-black">
            <div className="w-full bg-gray-50 dark:bg-black overflow-y-auto p-4">
                ...
                <div className="bg-white dark:bg-[#0d0d0d] dark:text-white rounded-lg shadow-sm p-4 mb-6">
                    ...
                    {sentFriendRequests.requests.length > 0 ? <div className="space-y-4">
                        {sentFriendRequests.requests.map((request: any) => (
                            ...
                            >
                                ...
                                <div className={`p-2 px-5 rounded-xl text-xs ${request?.status == "rejected" && "bg-red-700"} ${request?.status == "accepted" && "bg-green-500"}  ${request?.status == "pending" && "bg-yellow-500"}`}>
                                    {request?.status}
                                </div>
                            </div>
                        ))}
